...
const done=n=>localStorage.getItem(`zeyn-starters-completed-${n}`)==='1';
...
const last=+localStorage.getItem('zeyn-starters-last-test')||0;if(last){const cc=q('#continueCard');cc.classList.add('show');cc.href=`test${String(last).padStart(2,'0')}.html`;q('#continueTitle').textContent=`Starters Test ${last}`;}
...
return}
const a=q('#audio');if(!a)return;const n=+b.dataset.test;localStorage.setItem('zeyn-starters-last-test',n);
...
function sync(){const p=a.duration?a.currentTime/a.duration:0;bar.style.width=`${p*100}%`;q('#timeText').textContent=`${fmt(a.currentTime)} / ${fmt(a.duration)}`;q('#percentText').textContent=`${Math.round(p*100)}%`;q('#currentBig').textContent=fmt(a.currentTime);q('#durationBig').textContent=fmt(a.duration);localStorage.setItem(`zeyn-starters-position-${n}`,a.currentTime)}
a.addEventListener('loadedmetadata',()=>{const s=+localStorage.getItem(`zeyn-starters-position-${n}`)||0;if(s>0&&s<a.duration-3)a.currentTime=s;if(done(n)){complete.classList.add('show');status.textContent='Completed'}sync()});
a.addEventListener('timeupdate',sync);a.addEventListener('play',()=>{play.textContent='❚❚';wave.classList.add('playing');status.textContent='Playing'});a.addEventListener('pause',()=>{play.textContent='▶';wave.classList.remove('playing');if(!a.ended)status.textContent='Paused'});a.addEventListener('ended',()=>{localStorage.setItem(`zeyn-starters-completed-${n}`,'1');complete.classList.add('show');status.textContent='Completed'});
...
q('#offlineBtn').onclick=()=>{const x=document.createElement('a');x.href=a.src;x.download=`Starters Test ${n}.mp3`;x.click()};
