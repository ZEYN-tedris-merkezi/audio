...
(() => {
  ...
  function completedCount() {
    ...
    for (let i = 1; i <= TOTAL; i++) {
      if (localStorage.getItem(`flyers-completed-${i}`) === '1') count++;
    }
    return count;
  }

  function getLastTest() {
    return Number(localStorage.getItem('flyers-last-test') || 1);
  }
  ...
  function refreshIndex() {
    ...
    document.querySelectorAll('.test-card').forEach(card => {
      ...
      const isCompleted = localStorage.getItem(`flyers-completed-${testNumber}`) === '1';
  ...
  if (n) localStorage.setItem('flyers-last-test', String(n));
  ...
  const sync = () => {
    ...
    if (n) localStorage.setItem(`flyers-position-${n}`, String(audio.currentTime));
  };

  audio.addEventListener('loadedmetadata', () => {
    const saved = Number(localStorage.getItem(`flyers-position-${n}`) || 0);
    if (saved > 0 && saved < audio.duration - 3) audio.currentTime = saved;
    if (localStorage.getItem(`flyers-completed-${n}`) === '1') {
  ...
  audio.addEventListener('ended', () => {
    localStorage.setItem(`flyers-completed-${n}`, '1');
    complete.classList.add('show');
    statusText.textContent = 'Completed';
  });
